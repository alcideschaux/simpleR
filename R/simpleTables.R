...
descriptive.numerical <- function(x){
  ...
  knitr::kable(final.table, align = c("l", "c"))
}
...
descriptive.categorical <- function(x){
  ...
  knitr::kable(final.table, align = c("c", "c"))
}
...
descriptive.categorical.group <- function(x, y){
  ...
  matrix.final <- matrix(table.final, ncol = nlevels(y)*2,
    dimnames = list(c(levels(x)), c(levels(y)[1], "%", levels(y)[2], "%")))
  knitr::kable(matrix.final, align = c("c"))
}
...
logistic.table <- function(outcome, predictors, varlabels){
  ...
  OR.table <- cbind("Variables" = varlabels, "OR" = OR.center,
    "Lower 95% CI" = OR.low, "Higher 95% CI" = OR.high, "P value" = OR.p)
...
cox.table <- function(outcome, fu, predictors, varlabels){
  ...
  HR.table <- cbind("Variables" = varlabels, "HR" = HR.center,
    "Lower 95% CI" = HR.low, "Higher 95% CI" = HR.high, "P value" = HR.p)
